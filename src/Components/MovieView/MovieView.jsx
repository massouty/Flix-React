...
export function MovieView({ movie, onBackClick }) {
  ...
  const getFavoriteMoviesArray = (username) => {
    axios
      .get(`https://myflixdbapi.herokuapp.com/users/${username}`, {
  ...
  const addMovieToFavorites = (username, movieId) => {
    axios
      .post(`https://myflixdbapi.herokuapp.com/users/${username}/movies/${movieId}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((response) => {
        setFavoriteMovies(response.data.FavoriteMovies);
  ...
  const removeMovieFromFavorites = (username, movieId) => {
    axios
      .delete(
        `https://myflixdbapi.herokuapp.com/users/${username}/movies/${movieId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      )
  ...
  return (
    <div className="movie-view">
      ...
      <div className="movie-poster pb-3">
        <img width="250" src={movie.ImagePath} alt=" "/>
      </div>
